...
export function reportAiring(contentItemId: string, contentType: string, durationSec: number) {
  if (!isLiveOutput()) return;
  void post("/api/output/airing", { content_item_id: contentItemId, content_type: contentType, duration_sec: durationSec });
}
