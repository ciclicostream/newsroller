...
export function Placas() {
  ...
  const [kicker, setKicker] = useState("NOTICIAS");
  ...
  function usefulPost(p: CiclicoPost) {
    setKicker((p.categories[0] || "NOTICIAS").toUpperCase());
    setTitle(p.title.slice(0, T_MAX));
    setBody(p.excerpt.slice(0, B_MAX));
    setMediaUrl(p.image);
    setSource("somosciclico.com");
    setImportOpen(false);
  ...
  return (
    <>
      <div className="page-head">
        <div>
          <h1>Placas</h1>
          <p>Noticia genérica: escribila a mano o traela del sitio de Cíclico. Título obligatorio (admite **negrita**), foto opcional.</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "380px 1fr", gap: 20, alignItems: "start" }}>
        <form className="card" style={{ padding: 18 }} onSubmit={save}>
          ...
          <div className="field">
            <label>Volanta / categoría</label>
            <input value={kicker} onChange={(e) => setKicker(e.target.value.slice(0, 24))} placeholder="NOTICIAS" />
          </div>
          ...
          <div className="field">
            ...
            <textarea value={body} onChange={(e) => setBody(e.target.value.slice(0, B_MAX))} rows={3} maxLength={B_MAX} />
          ...
          <div className="field">
            ...
            <input value={source} onChange={(e) => setSource(e.target.value)} placeholder="somosciclico.com" />
          </div>

          <div className="field">
            <label>Duración (segundos)</label>
            <input type="number" min={2} value={dur} onChange={(e) => setDur(Math.max(2, Number(e.target.value) || 9))} />
          </div>
            ...
            return (
              <div key={it.id} className="card" style={{ padding: 16, display: "flex", gap: 16, alignItems: "center" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, color: "#6b7688", fontWeight: 700 }}>{(d.kicker || "NOTICIAS").toUpperCase()}</div>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            {loadingPosts ? (
              ...
              <div className="imp-list">
                {posts.map((p) => (
                  <button key={p.id} className="imp-item" onClick={() => usefulPost(p)}>
